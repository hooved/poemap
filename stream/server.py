from comms import receive_int, receive_array, ClientHeader
import numpy as np
from PIL import Image
import asyncio, os
from models import AttentionUNet, ViT
from tinygrad import dtypes, Tensor, TinyJit
from training_data import tokenize_minimap, tensorize_tokens
    ...
    if header == ClientHeader.PROCESS_MINIMAP:
      try:
        ...
        if np.any(mask != 0):
          ...
          tokens, pe = tensorize_tokens(tokens, models["ViT"].embed_dim)
...
async def run_server(hostname, port: int):
  models = {
    ...
    "ViT": ViT("ViT6_219_15", num_classes=9, max_tokens=128, layers=3, embed_dim=256, num_heads=4).load(),
  }
  ...
  print(f"ViT warmup argmax: {warmup}")
